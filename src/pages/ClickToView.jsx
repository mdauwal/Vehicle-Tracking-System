...
const ClickToView = () => {
  ...
  const { selectedIncidenceIndex, handleCardClick, showDetails, goBackToCards } = useContext(IncidenceContext);
  ...
  return (
    ...
    <div className='grid grid-cols-1 md:grid-cols-5 md:p-5'>
      ...
      <div className={`col-span-1 p-5 ${showDetails ? 'hidden md:block' : ''}`}>
        ...
        <div className='mb-10'>
          {/* fix the title */}
          <Link to={'someplace'} className='flex items-center gap-2 mb-5'>
            <FaArrowLeft /> Back
          </Link>
        ...
        <div className='overflow-y-scroll'>
          <IncidenceCards selectedIncidenceIndex={selectedIncidenceIndex}
          onCardClick={handleCardClick} />
          </div>
      </div>

      <div className={`col-span-4 p-5 ${!showDetails ? 'hidden md:block' : ''}`}>
        <span className='flex justify-between mb-5'>
          <Link to={'someplace'} className='flex items-center gap-2'>
            <FaArrowLeft /> Back
          </Link>
